...
let subtasks = [];
let assignContainer;
let contactsDropdown = new Dropdown(
  "arrow-contacts",
  "contact-dropdown-container"
);
let categoryDropdown = new Dropdown(
  "arrow-category",
...
function toggleContactSelection(index) {
  arrayOfFilterContact[index]["selected"] =
    !arrayOfFilterContact[index]["selected"];
  renderContacts();
  renderBadges();
}
...
function enableSubtaskInput() {
  ...
  document
    .getElementById("addTask-delete-accept-container")
...
function enableEditSubtask(event, index) {
  ...
  let input = document.getElementById(
    `addTask-subtask-listElement-input_${index}`
  );
  input.readOnly = false;
  input.classList.add("inputField-enabled");
  input.focus();
...
function removePseudo(index) {
  let subtaskElement = document.getElementById(
    `addTask-subtask-element_${index}`
  );
